...
namespace Convai.Scripts.Runtime.Features
{
    ...
    public enum ActionChoice
    {
        ...
        PickUp,
        Drop,
        // new, to steal key card in phase 2
        ParalyzeGuard
    }
    ...
    {
        [Header("Heist / Safe Route Hooks")]
        [Tooltip("All main patrol guards Sakura is allowed to stun for the keycard route.")]
        public GuardDronePatrol[] stunnableGuards;
        public float guardStunDuration = 7f;
       ...
       private void Reset()
{
    actionMethods = new ActionMethod[]
    {
        ...
        // NEW: the thing Convai will call when player chooses the safe keycard route
        new() { action = "Paralyze Guard", actionChoice = ActionChoice.ParalyzeGuard }
    };
}
        ...
        private IEnumerator DoAction(ConvaiAction action)
{
    switch (action.Verb)
    {
        ...
        //  NEW CASE
        case ActionChoice.ParalyzeGuard:
            yield return ParalyzeGuardForKeycard(action.Target);
            break;
...
private GuardDronePatrol FindGuardForTarget(GameObject target)
{
    // Try: 1) direct from target
    if (target != null)
    {
        var fromTarget = target.GetComponentInParent<GuardDronePatrol>();
        if (fromTarget != null)
            return fromTarget;
    }

    if (stunnableGuards == null || stunnableGuards.Length == 0)
        return null;

    // Decide an origin point for "closest guard":
    // - if we have a target, use that
    // - else try player
    // - else fall back to Sakura herself
    Vector3 origin;
    ...
    else
    {
        ...
        if (playerGO != null)
            origin = playerGO.transform.position;
        else if (_currentNPC != null)
            origin = _currentNPC.transform.position;
        else
            origin = stunnableGuards[0].transform.position;
    }

    GuardDronePatrol bestGuard = null;
    float bestDist = float.MaxValue;

    foreach (var g in stunnableGuards)
    {
        if (g == null) continue;
    ...
    {
        ConvaiLogger.DebugLog(
            $"[ConvaiActionsHandler] ParalyzeGuard choosing nearest guard: {bestGuard.name}",
            ConvaiLogger.LogCategory.Actions
        );
    }
...
private IEnumerator ParalyzeGuardForKeycard(GameObject target)
{
    ActionStarted?.Invoke("ParalyzeGuard", target);

    GuardDronePatrol guard = FindGuardForTarget(target);
    if (guard == null)
    {
        ConvaiLogger.Warn("ParalyzeGuard called but no valid GuardDronePatrol found!", ConvaiLogger.LogCategory.Actions);
        yield break;
    }

    // Sakura talks before stunning
    if (_currentNPC != null)
    {
        _currentNPC.InterruptCharacterSpeech();
        _currentNPC.TriggerSpeech(
            "[WHISPER][EXCITED] Okay, safe route it is. I'll paralyze that guard for five seconds – " +
            "when he locks up, sneak in and knock him out to grab the keycard."
        );
    }

    // Stun the chosen guard (vibrate + stop moving – implemented in GuardDronePatrol)
    guard.ApplyStun(guardStunDuration);

    // Optional: wait so this action owns the stun window
    yield return new WaitForSeconds(guardStunDuration);

    ActionEnded?.Invoke("ParalyzeGuard", target);
}
