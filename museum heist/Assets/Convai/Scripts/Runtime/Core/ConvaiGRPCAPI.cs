...
private void ProcessUserQuery(GetResponseResponse result)
{
    ...
    {
        // Build current live transcript for UI
        _currentTranscript = _isFinalUserQueryTextBuffer + result.UserQuery.TextData;

        // Buffer final chunks so we can log the complete utterance
        if (result.UserQuery.IsFinal)
            _isFinalUserQueryTextBuffer += result.UserQuery.TextData;

        // When the utterance is complete, log it once
        if (result.UserQuery.EndOfResponse)
        {
            string finalUtterance = _isFinalUserQueryTextBuffer.Trim();

            if (!string.IsNullOrWhiteSpace(finalUtterance))
            {
                GameRunLogger.Instance?.Log("Dialogue", "Dialogue_Player_Spoken", "Player", finalUtterance);
            }
