    ...
    private void InitializeComponents()
{
    _convaiChatUIHandler =
        UnityEngine.Object.FindAnyObjectByType<ConvaiChatUIHandler>();

    _convaiCrosshairHandler =
        UnityEngine.Object.FindAnyObjectByType<ConvaiCrosshairHandler>();
    ...
    StartCoroutine(InitializeActionsHandler());
        ...
        {
        //  Log typed text the player sends
        if (!string.IsNullOrWhiteSpace(text))
        {
            GameRunLogger.Instance?.Log("Dialogue", "Dialogue_Player_Typed", "Player", text);
        }
        ...
        private void CreateAndAddAudioClip(bool isFinal)
        {
            ...
            //  Log NPC dialogue only when final, to avoid duplicates
            if (isFinal && !string.IsNullOrWhiteSpace(mergedTranscript))
            {
                GameRunLogger.Instance?.Log("Dialogue", "Dialogue_NPC", characterName, mergedTranscript);
            }
